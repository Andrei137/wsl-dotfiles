...
mapn(m.vi, '>', '>gv', 'Shift right')
mapn(m.vi, '<', '<gv', 'Shift left')
mapn(m.i, '<C-h>', '<C-w>', 'Delete word')
map(m.v, 'J', ":m '>+1<CR>gv=gv", { silent = true })
map(m.v, 'K', ":m '<-2<CR>gv=gv", { silent = true })
...
mapn(m.n, '<C-f>', ':Format <CR> :w<CR>', 'Format and save')

-- Quit
mapn(m.ni, '<C-q>', ':q <CR>', 'Quit file')
mapn(m.ni, '<C-Q>', ':q! <CR>', 'Quit without saving')
...
mapn(m.n, '<leader>b', ':enew <CR>', 'New buffer')
